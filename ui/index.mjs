...
return{summaries:n,support:r}}var Do=String.raw`
  ...
  .ow-beta {
    /* Pin the line box; without this the badge inherits the title's tall
       line-height and renders as a tall oval. */
    line-height: 1;
    padding: 2px 9px;
...
`${I.kind}:${I.id}`))})]}),$&&Y&&w("div",{className:"ow-row-mismatch",children:[w("span",{className:"ow-truncate",children:[
...
bn,{})),Ie=le([]),Vt=le(!1),[Le,Cn]=_([]),[Ue,ur]=_(()=>re(Vo)),[Rt,It]=_(null),[pr,gr]=_(()=>re(jo,null)??!0),[An,Wn]=_(
...
[]);let $n=V(()=>bo(_o(W??{slots:[],approvals:[],agents:[],workflows:[],crons:[],artifacts:[],loops:[]},be,I,ve,An,X),je),
[W,I,ve,An,je,X]),Pt=V(()=>yo($n,Nt,C),[$n,Nt,C]),A=V(()=>Pt.items.filter(o=>ko(o)),[Pt]),Et=V(()=>gn(A),[A]),Tn=V(()=>{
let o={};for(let c of A){if(c.state!=="done"||!c.sessionKey)continue;let g=o[c.sessionKey];g?g.push(c.title):o[c.sessionKey]=
[c.title]}return o},[A]),Ce=V(()=>A.find(o=>o.id===L)??null,[A,L]),ht=V(()=>s==="all"?A:A.filter(o=>o.state===s),[s,A]),
...
St]),Ye=Ce&&!Ce.permissionId?Ce:null,ce=V(()=>Ko(A,Le,X,ge.current,Z.pairs),[A,Le,X,Z]);H(()=>{let o=Ro(ce.filter(c=>c.name===
...
[l,W,A,Le]),Ir=D(async(o,c)=>{try{await t.current.patch(`/api/chat/slots/${encodeURIComponent(o)}/title`,{title:c}),B()}catch{}},
...
onMergeGoal:Br,initiativeBlocks:ce,initiatives:Le,onRenameSession:(o,c)=>{Ir(o,c)},semanticWhy:Z.why,goalNames:Ke,collapsedInitiatives:Ue,
...
embed",children:a($s,{slotKey:dt,frameless:!0,startAtBottom:!0,placeholder:me?"Instruction for this goal\u2026":Ye?.sessionKey&&

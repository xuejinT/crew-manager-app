...
`${C.kind}:${C.id}`))})]}),L&&Y&&w("div",{className:"ow-row-mismatch",children:[w("span",{className:"ow-truncate",children:[
...
bn,{})),Ce=le([]),Vt=le(!1),[$e,In]=_([]),[Ue,ur]=_(()=>re(Vo)),[Rt,Ct]=_(null),[pr,gr]=_(()=>re(jo,null)??!0),[An,Wn]=_(
...
[]);let Ln=V(()=>bo(_o(W??{slots:[],approvals:[],agents:[],workflows:[],crons:[],artifacts:[],loops:[]},be,C,ve,An,X),je),
[W,C,ve,An,je,X]),Pt=V(()=>yo(Ln,Nt,I),[Ln,Nt,I]),A=V(()=>Pt.items.filter(o=>ko(o)),[Pt]),Et=V(()=>gn(A),[A]),Tn=V(()=>{
let o={};for(let c of A){if(c.state!=="done"||!c.sessionKey)continue;let g=o[c.sessionKey];g?g.push(c.title):o[c.sessionKey]=
[c.title]}return o},[A]),Ie=V(()=>A.find(o=>o.id===$)??null,[A,$]),ht=V(()=>s==="all"?A:A.filter(o=>o.state===s),[s,A]),
...
St]),Ye=Ie&&!Ie.permissionId?Ie:null,ce=V(()=>Ko(A,$e,X,ge.current,Z.pairs),[A,$e,X,Z]);H(()=>{let o=Ro(ce.filter(c=>c.name===
...
[l,W,A,$e]),Cr=D(async(o,c)=>{try{await t.current.patch(`/api/chat/slots/${encodeURIComponent(o)}/title`,{title:c}),B()}catch{}},
...
onMergeGoal:Br,initiativeBlocks:ce,initiatives:$e,onRenameSession:(o,c)=>{Cr(o,c)},semanticWhy:Z.why,goalNames:Ke,collapsedInitiatives:Ue,
...
embed",children:a(Ls,{slotKey:dt,frameless:!0,startAtBottom:!0,slotControls:!0,placeholder:me?"Instruction for this goal\
